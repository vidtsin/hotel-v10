...
class account_change_check_wizard(models.TransientModel):
    _name = 'account.change.check.wizard2'

    @api.model
    def get_domain(self):
        return [('cashin_out_id','=',self._context.get('active_id')),('state','=','handed')]

    def get_original_check(self):
        return self.original_check_id.browse(self._context.get('active_id'))

    original_check_id2 = fields.Many2one(
        'account.check',
        'Original Check',
        required=True,
        domain=get_domain,
    ...
    @api.depends('original_check_id2')
    @api.onchange('original_check_id2')
    def change_original_check2(self):
        self.original_check_id = self.original_check_id2
    ...
    def change(self):
        ...
        if number != "0":
            vals = {
                ...
                'amount_cash': self.original_check_id.amount,
            }
            if self.original_check_id.type == 'issue_check':
            ...
            cash = self.original_check_id.cashin_out_id
            cash.check_line_ids = [(3, self.original_check_id.id)]
            cash.check_line_ids = [(4, new_check.id)]
