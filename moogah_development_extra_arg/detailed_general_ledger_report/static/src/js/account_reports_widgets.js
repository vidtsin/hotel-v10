odoo.define('detailed_general_ledger_report.ReportWidget', function (require) {
'use strict';
...
var ReportWidget = Widget.extend({
    ...
    unfold: function(e) {
        ...
        var active_tag_id = $(e.target).parents('tr').find('td.o_account_reports_unfoldable').data('tag-id');
        return this.context_model.call('add_line', [[parseInt(context_id, 10)], parseInt(active_id, 10)]).then(function () { // First add the line to the model
            ...
            if (!isLoaded) { // If the lines have not yet been loaded
                ...
                new Model('account.report.context.common').call('get_full_report_name_by_report_name', [report_name]).then(function (result) {
                    ...
                    if (report_name === 'detailed_general_ledger') { // Fetch the report_id first if needed
                        reportObj.call('get_lines', [parseInt(context_id, 10), parseInt(active_id, 10),parseInt(active_tag_id, 10)], {context : self.odoo_context}).then(f);
                    }
                    else {
