...
class AccountPaymentGroup(models.Model):
    ...
    aux_payment_difference = fields.Float(
        compute='_compute_aux_payment_difference',
        # TODO rename field or remove string
        # string='Remaining Residual',
        readonly=True,
        string="Payment Difference Without Rounding",
    )

    aux_payments_amount = fields.Float(
        compute='_compute_aux_payments_amount',
        string='Amount Without Rounding',
        translate=True,
    )

    @api.multi
    @api.depends('to_pay_amount', 'aux_payments_amount')
    def _compute_aux_payment_difference(self):
        for rec in self:
            rec.aux_payment_difference = rec.to_pay_amount - rec.aux_payments_amount

    @api.multi
    @api.depends('payment_ids.aux_amount_company_currency')
    def _compute_aux_payments_amount(self):
        for rec in self:
            rec.aux_payments_amount = sum(rec.payment_ids.mapped(
                'aux_amount_company_currency'))
    ...
    def _compute_tot_currency(self):
        for rec in self:
            # TODO:Pago de Facturas USD en USD
            ...
            # TODO:Pago de Facturas ARS en USD
            if rec.inv_currency_id.id != rec.currency2_id.id == rec.currency_id.id:
    ...
    def _compute_paid_currency(self):
        for rec in self:
            ...
            rec.paid_payment_currency = rec.paid_company_currency
            if rec.manual_currency_rate > 0 and rec.payment_ids and rec.payment_ids[0].currency_id != rec.currency_id:
