...
class AccountMoveLine(models.Model):
    ...
    @api.model
    def compute_amount_fields_custom(self, obj, amount, src_currency,
                                     company_currency, invoice_currency=False,
                                     currency_rate=False):
...
class AccountPayment(models.Model):
    ...
    currency2_id = fields.Many2one('res.currency',
                                   related='payment_group_id.currency2_id',
                                   string="Payment Currency")
    company_currency_id = fields.Many2one('res.currency',
                                          related='payment_group_id.currency_id',
                                          string="Company Currency")
    pay_amount_currency2_id = fields.Monetary(
        string='Payment Amount in Payment Currency',
        compute='_compute_pay_amount_currency2_id',
        currency_field='currency2_id')

    currency_rate = fields.Float('Currency Rate', readonly=True,
                                 related='payment_group_id.manual_currency_rate',
                                 digits=(12, 6))
    to_pay_payment_currency = fields.Monetary(
        string='To Pay in Payment Currency',
        compute='_compute_to_pay_currency',
        currency_field='currency2_id')
    to_pay_company_currency = fields.Monetary(
        string='To Pay in Company Currency',
        compute='_compute_to_pay_currency',
        currency_field='company_currency_id')

    aux_amount_company_currency = fields.Float(
        string='Payment Amount on Company Currency Without Rounding',
        translate=True,
        compute='_compute_aux_amount_company_currency',
    )

    @api.one
    @api.depends('amount', 'currency_id', 'company_id.currency_id',
                 'manual_currency_rate_active')
    def _compute_aux_amount_company_currency(self):
        payment_currency = self.currency_id
        company_currency = self.company_id.currency_id
        manual_currency_rate_active = self.manual_currency_rate_active
        if self._context != None and 'default_manual_currency_rate_active' in self._context:
            manual_currency_rate_active = self._context[
                'default_manual_currency_rate_active']
        # if self.payment_group_id and payment_currency == self.payment_group_id.currency2_id and self.manual_currency_rate_active:
        if self.payment_group_id and payment_currency != company_currency and manual_currency_rate_active:
            aux_amount_company_currency = self.amount / self.manual_currency_rate
        elif payment_currency and payment_currency != company_currency:
            aux_amount_company_currency = self.currency_id.with_context(
                date=self.payment_date).compute(
                self.amount, self.company_id.currency_id)
        else:
            aux_amount_company_currency = self.amount
        sign = 1.0
        if (
            ...
            sign = -1.0
        self.aux_amount_company_currency = aux_amount_company_currency * sign

    @api.multi
    @api.depends('pay_amount_currency2_id', 'amount', 'currency_id',
                 'readonly_amount2')
    def _compute_to_pay_currency(self):
        for rec in self:
            total = sum(rec.payment_group_id.mapped('payment_ids').mapped(
                'pay_amount_currency2_id'))
            rec.to_pay_payment_currency = total
            rec.to_pay_company_currency = total
            if rec.currency2_id != rec.company_currency_id:
    ...
    def _compute_pay_amount_currency2_id(self):
        for rec in self:
            if rec.readonly_amount2 and rec.readonly_amount2 == rec.amount and rec.currency_id.id == rec.currency2_id.id:
                ...
                rec.pay_amount_currency2_id = rec.amount
            elif rec.currency_id.id == rec.company_currency_id.id != rec.currency2_id.id and rec.currency_rate:
                rec.pay_amount_currency2_id = rec.amount * rec.currency_rate
            elif rec.currency_id.id != rec.currency2_id.id == rec.company_currency_id.id:
                rec.pay_amount_currency2_id = rec.amount / rec.currency_rate
            else:
                currency = rec.currency2_id.with_context(
                    date=rec.payment_date or fields.Date.context_today(rec))
                rec.pay_amount_currency2_id = rec.currency_id.compute(
                    rec.amount, currency)

    @api.one
    @api.depends('amount', 'currency_id', 'company_id.currency_id', 'manual_currency_rate_active')
    def _compute_amount_company_currency(self):
        payment_currency = self.currency_id
        company_currency = self.company_id.currency_id
        manual_currency_rate_active =  self.manual_currency_rate_active
        if self._context != None and 'default_manual_currency_rate_active' in self._context:
            manual_currency_rate_active = self._context['default_manual_currency_rate_active']
        # if self.payment_group_id and payment_currency == self.payment_group_id.currency2_id and self.manual_currency_rate_active:
        if self.payment_group_id and payment_currency != company_currency and manual_currency_rate_active:
    ...
    def _create_payment_entry(self, amount):
        ...
        if self.manual_currency_rate_active:
            ...
            debit, credit, amount_currency, currency_id = aml_obj.with_context(
                date=self.payment_date).compute_amount_fields_custom(self,
                                                                     amount,
                                                                     self.currency_id,
        ...
        if self.payment_difference_handling == 'reconcile' and self.payment_difference:
            writeoff_line = self._get_shared_move_line_vals(0, 0, 0, move.id,
                                                            False)
        ...
        if not self.currency_id != self.company_id.currency_id and not currency_rate:  #
            amount_currency = 0
        liquidity_aml_dict = self._get_shared_move_line_vals(credit, debit,
                                                             -amount_currency,
                                                             move.id, False)
    ...
    def onchange_amount_in_payment_currency(self):
        amount = self.payment_group_id.aux_payment_difference
        if self.currency_id != self.payment_group_id.currency_id == self.payment_group_id.inv_currency_id:
            amount = amount * self.payment_group_id.manual_currency_rate
        if self.currency_id == self.payment_group_id.currency_id != self.payment_group_id.inv_currency_id:
            amount = amount / self.payment_group_id.manual_currency_rate
        self.amount = amount
