odoo.define('origin.document.payment', function (require) {
"use strict";
...
var Payment = core.form_widget_registry.get('payment');
        ...
        if (info !== false) {
            ...
            _.each(this.$('.js_payment_info'), function(k, v){
                ...
                $(k).on('shown.bs.popover', function(event){
                    ...
                    $(this).parent().find('.js_open_payment_origin').click(function(){
                        var move_id = parseInt($(this).attr('move-id'))
                        if (move_id !== undefined && move_id !== NaN){
                            //Open form view of account.move with id = move_id
                            if (info.content[v].type == 'payment'){
                                self.do_action({
                                    type: 'ir.actions.act_window',
                            ...
                            }else if (info.content[v].type == 'invoice'){
                                self.do_action({
                                    type: 'ir.actions.act_window',
