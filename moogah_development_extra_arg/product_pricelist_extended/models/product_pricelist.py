...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False,
                            uom_id=False):
        """ Redefined function. If the price is calculated base on new product
        fields, returned values are based on new currency field value.
        ...
        for product, qty, partner in products_qty_partner:
            ...
            if qty_uom_id != product.uom_id.id:
                try:
                    qty_in_product_uom = self.env['product.uom'].browse(
                        [self._context['uom']])._compute_quantity(qty, product.uom_id)
                except UserError:
            ...
            for rule in items:
                ...
                if rule.base == 'pricelist' and rule.base_pricelist_id:
                    ...
                    # [INIT] customized code
                    new_fields_base_on = False
                    if rule.base in ['last_po_cost', 'lp_base', 'cost']:
                        price = product.product_currency_id.compute(price,
                                                            self.currency_id,
                                                            round=False)
                        new_fields_base_on = True
                    # [END] customized code
...
class PricelistItem(models.Model):
    ...
    base = fields.Selection(selection_add=[('last_po_cost', 'Last PO cost'),
                                           ('lp_base', 'LP base'),
                                           ('cost', 'Cost')])
