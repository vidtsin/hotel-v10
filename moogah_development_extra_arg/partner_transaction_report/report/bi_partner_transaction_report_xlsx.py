...
class PartnerTransactionReportXlsx(models.AbstractModel):
    _name = 'report.partner_transaction_report.report_xlsx'
    ...
    def generate_xlsx_report(self, data):
        ...
        move_lines = self.get_lines(data)
        if not move_lines:
            raise UserError(_('There is no information for the selected filters.'))

        # -- ESTRUCTURA DE LA HOJA DE CALCULO--
        try:
            name_sheet = 'Partner Transaction Report'
            output = StringIO()
            workbook = xlsxwriter.Workbook(output, {'in_memory': True})
            sheet = workbook.add_worksheet(name_sheet)

            # DEFINIMOS LOS ESTILOS A APLICAR EN LAS CELDAS
            title_style = workbook.add_format({'font_name': 'Arial', 'bold': True})
            upper_line_style = workbook.add_format({'font_name': 'Arial', 'bottom': 2, 'bold': True})
            domain_style_left = workbook.add_format({'font_name': 'Arial', 'bold': True})
            domain_style_right = workbook.add_format({'font_name': 'Arial', 'top': 2, 'bold': True, 'align': 'right'})
            line_style_right = workbook.add_format({'font_name': 'Arial', 'bold': True, 'align': 'right'})
            def_style = workbook.add_format({'font_name': 'Arial'})
            style_right = workbook.add_format({'font_name': 'Arial', 'align': 'right'})

            # CON ESTO DEFINIMOS EL AHCHO DE LAS COLUMNAS
            sheet.set_column('A:A', 20)

            sheet.write('A1', '', title_style)
            sheet.write('B1', _('Date'), title_style)
            sheet.write('C1', _('Doc Type'), title_style)
            sheet.write('D1', _('Number'), title_style)
            sheet.write('E1', _('Reference'), title_style)
            sheet.write('F1', _('Manual Rate'), title_style)
            sheet.write('G1', _('Amount in Currency'), title_style)
            sheet.write('H1', _('Debit'), title_style)
            sheet.write('I1', _('Credit'), title_style)
            sheet.write('J1', _('Balance'), title_style)

            fila = 2

            for partner in data['form']['partner_ids']:
                if partner in move_lines:
                    sheet.write('A' + str(fila), move_lines[partner]['title'], upper_line_style)
                    fila += 1
                    for currency in move_lines[partner]['currency']:
                        sheet.write('A' + str(fila), currency['currency']['title'], domain_style_left)
                        fila += 1
                        sheet.write('A' + str(fila), currency['currency']['initial_balance']['title'], def_style)
                        sheet.write('H' + str(fila), currency['currency']['initial_balance']['debit'], style_right)
                        sheet.write('I' + str(fila), currency['currency']['initial_balance']['credit'], style_right)
                        sheet.write('J' + str(fila), currency['currency']['initial_balance']['balance'], style_right)
                        fila += 1
                        for currency_line in currency['currency']['lines']:
                            if currency_line['line_type'] != 'total':
                                name_style = domain_style_left if currency_line[
                                                              'line_type'] != 'payment_line' else def_style
                                number_style = line_style_right if currency_line[
                                                                'line_type'] != 'payment_line' else style_right
                                sheet.write('A' + str(fila), currency_line['number'], name_style)
                                sheet.write('B' + str(fila), currency_line['date'], name_style)
                                sheet.write('C' + str(fila), currency_line['doc_type'] or '', name_style)
                                sheet.write('D' + str(fila), currency_line['number'], name_style)
                                sheet.write('E' + str(fila), currency_line['reference'] or '', name_style)
                                sheet.write('F' + str(fila), currency_line['currency_rate'], number_style)
                                sheet.write('G' + str(fila), currency_line['amount_in_currency'], number_style)
                                sheet.write('H' + str(fila), currency_line['debit'], number_style)
                                sheet.write('I' + str(fila), currency_line['credit'], number_style)
                                sheet.write('J' + str(fila), currency_line['balance'], number_style)
                                fila += 1
                            if currency_line['line_type'] == 'total':
                                sheet.write('A' + str(fila), currency_line['title'], def_style)
                                sheet.write('H' + str(fila), currency_line['debit'], style_right)
                                sheet.write('I' + str(fila), currency_line['credit'], style_right)
                                sheet.write('J' + str(fila), currency_line['balance'], style_right)
                                fila += 1
                        sheet.write('A' + str(fila), currency['currency']['total']['title'], domain_style_left)
                        sheet.write('H' + str(fila), currency['currency']['total']['debit'], domain_style_right)
                        sheet.write('I' + str(fila), currency['currency']['total']['credit'], domain_style_right)
                        sheet.write('J' + str(fila), currency['currency']['total']['balance'], domain_style_right)
                        fila += 2
                    fila += 1
            ...
            raise UserError(_('An error has occurred generating XLS file...'), str(e))
