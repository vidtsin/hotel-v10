...
class AccountMoveLine(models.Model):
    ...
    check_id = fields.Many2one('account.check', related='payment_id.check_id', string='Check', readonly=True, store=True)
    check_state = fields.Selection(related='check_id.state', string='Check State', readonly=True, store=True)
    bank_id = fields.Many2one('res.bank', related='check_id.bank_id', string='Bank', readonly=True, store=True)
    check_issue_date = fields.Date(related='check_id.issue_date', string='Issue Date', readonly=True, store=True)
    check_deposit_id = fields.Many2one('account.check.deposit', string='Check Deposit', copy=False)
    check_amount = fields.Monetary(related='check_id.amount', string='Amount', readonly=True, store=True)
    check_payment_date = fields.Date(related='check_id.payment_date', string='Payment Date', readonly=True, store=True)
