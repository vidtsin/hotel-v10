...
class HotelFolio(models.Model):
    ...
    def compute_no_days(self):
        for rec in self:
            if rec.checkin_date and rec.checkout_date:
                checkin_date = datetime.strptime(rec.checkin_date,
                                                 DEFAULT_SERVER_DATE_FORMAT)
                checkout_date = datetime.strptime(rec.checkout_date,
                                                  DEFAULT_SERVER_DATE_FORMAT)
    ...
    def compute_check_in_out(self):
        for rec in self:
            ...
            rec.update({
                        'checkin_date': checkin_list and min(checkin_list)
                                        or datetime.today(),
        ...
        for folio in self:
            for picking in folio.picking_ids:
                if picking.state not in ['done', 'cancel']:
                    wiz_data = picking.do_new_transfer()
                ...
                for line in lines:
                    folio._cr.execute("""SELECT room_number_id FROM
                                ...
                                """,(line.checkin_date, line.checkout_date, line.id,
                                     line.room_number_id.id, line.company_id.id))
...
class HotelFolioLine(models.Model):
    ...
    def onchange_folio_date(self):
        if self.checkin_date and self.checkout_date:
            ...
            self.room_id = False
            checkin_date = datetime.strptime(self.checkin_date,
                                             DEFAULT_SERVER_DATE_FORMAT)
            checkout_date = datetime.strptime(self.checkout_date,
                                             DEFAULT_SERVER_DATE_FORMAT)
            dur = checkout_date - checkin_date
    ...
    def _get_display_price(self, product):
        ...
        product_context = dict(self.env.context, partner_id=self.folio_id.partner_id.id, date=self.folio_id.date_order, uom=self.product_uom.id)
        final_price, rule_id = self.folio_id.pricelist_id.with_context(product_context).get_product_price_rule(self.product_id, self.product_uom_qty or 1.0, self.folio_id.partner_id)
        base_price, currency_id = self.with_context(product_context)._get_real_price_currency(product, rule_id, self.product_uom_qty, self.product_uom, self.folio_id.pricelist_id.id)
        if currency_id != self.folio_id.pricelist_id.currency_id.id:
            base_price = self.env['res.currency'].browse(currency_id).with_context(product_context).compute(base_price, self.folio_id.pricelist_id.currency_id)
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            line.tax_id = fpos.map_tax(taxes, line.product_id, line.folio_id.partner_shipping_id) if fpos else taxes
...
class HotelServices(models.Model):
    ...
    @api.model
    def create(self, vals):
        res = super(HotelServices, self).create(vals)
    ...
    @api.multi
    def write(self, vals):
        res = super(HotelServices, self).write(vals)
        for rec in self:
            if vals.get('taxes_id'):
