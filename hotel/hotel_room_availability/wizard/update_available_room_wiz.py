...
class UpdateAvailableRoomQty(models.TransientModel):
    ...
    date_from = fields.Date(
        string='Date From', default=datetime.today(), required=True)
    date_to = fields.Date(
    ...
    def update_room_qty(self):
        ...
        for rec in self:
            date_to = datetime.strptime(
                rec.date_to, DEFAULT_SERVER_DATE_FORMAT)
            date_from = datetime.strptime(
                rec.date_from, DEFAULT_SERVER_DATE_FORMAT)
                    ...
                    if ttl_qty > line.hotel_room_id.rooms_qty:
                        raise ValidationError(_
                                              ("The room %s can not be\
                                                updated. The maximum\
                                                quantity is %d" %
                                               (line.hotel_room_id.name,
                                                line.hotel_room_id.rooms_qty)))

                    if line_rec:
                        line_rec.write({'room_qty': ttl_qty})
                    else:
                        vals = {
                        ...
                        availability_line_obj.create(vals)
