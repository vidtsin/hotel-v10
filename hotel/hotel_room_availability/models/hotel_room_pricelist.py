...
class RoomPricelist(models.Model):
    ...
    def _default_date_to(self):
        return (datetime.today() + relativedelta(days=+15)).strftime('%Y-%m-%d')

    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('room.pricelist'),
                                 required=True)
    ...
    room_pricelist_ids = fields.One2many('room.pricelist.line',
                                         'room_pricelist_id',
                                         'Rooms Pricelist')
            ...
            if company_id:
                self.env.cr.execute('''
                    ...
                    WHERE (date_from <= %s and %s <= date_to)
                        AND company_id=%s
                        AND id <> %s''',
                    (rec.date_to, rec.date_from, company_id.id, rec.id))
                if any(self.env.cr.fetchall()):
                    raise ValidationError(_('''You can't create 2 Pricelist that overlap!.'''))
...
class RoomPricelistLine(models.Model):
    ...
    room_pricelist_id_computed = fields.Many2one('room.pricelist',
                                                 string='Room Pricelist',
                                                 compute='_compute_room_pricelist',
                                                 index=True,
                                                 ondelete='cascade',
                                                 search='_search_room_pricelist')
    room_pricelist_id = fields.Many2one('room.pricelist',
                                        compute='_compute_room_pricelist',
                                        store=True)
...
class HotelReservationLine(models.Model):
    ...
    def get_price_from_rateplan(self, room):
        for rec in self:
            check_out_date = datetime.strptime(rec.checkout, DEFAULT_SERVER_DATE_FORMAT) +\
    ...
    def onchange_room_id(self):
        ...
        if self.reservation_id.pricelist_id and self.reservation_id.partner_id:
            if self.get_price_from_rateplan(room):
                vals['price_unit'] = self.env['account.tax']._fix_tax_included_price_company(self.get_price_from_rateplan(room),
                                                                                             room.taxes_id, self.tax_id,
                                                                                             self.company_id)
            else:
                vals['price_unit'] = self.env['account.tax']._fix_tax_included_price_company(self._get_display_price(room),
                                                                                             room.taxes_id, self.tax_id,
                                                                                             self.company_id)
        self.update(vals)
    ...
    def room_qty_change(self):
        ...
        if self.reservation_id.pricelist_id and self.reservation_id.partner_id:
            ...
            if self.get_price_from_rateplan(room):
                self.price_unit = self.env['account.tax']._fix_tax_included_price_company(self.get_price_from_rateplan(room),
                                                                                      room.taxes_id, self.tax_id,
                                                                                      self.company_id)
            else:
...
class HotelFolioLine(models.Model):
    ...
    @api.multi
    def get_price_from_rateplan(self, room):
        rateplan_line_obj = self.env['room.pricelist.line']
        for rec in self:
            check_out_date = datetime.strptime(rec.checkout_date, DEFAULT_SERVER_DATE_FORMAT) +\
                                relativedelta(days=-1)
            if room:
...
#            vals['product_uom_qty'] = 1.0
        ...
        if self.folio_id.pricelist_id and self.folio_id.partner_id:
            if self.get_price_from_rateplan(self.room_id):
                vals['price_unit'] = self.env['account.tax']._fix_tax_included_price_company(self.get_price_from_rateplan(self.room_id),
