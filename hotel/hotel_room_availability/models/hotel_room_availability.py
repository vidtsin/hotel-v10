...
class RoomAvailability(models.Model):
    ...
    room_availability_ids = fields.One2many('room.availability.line',
...
class HotelReservationLine(models.Model):
    ...
    def check_room_closing_status(self, room, checkin=False, checkout=False):
        ...
        if checkin != checkout:
            check_out_date = (datetime.strptime(
                self.checkout, DEFAULT_SERVER_DATE_FORMAT) +
    ...
    def get_available_room_qty(self, room, checkin, checkout):
        ...
        if self.get_qty_from_avaliability(room, checkin, checkout):
            available_room_qty = self.get_qty_from_avaliability(
...
class HotelFolioLine(models.Model):
    ...
    def check_room_closing_status(self, room):
        for rec in self:
            check_out_date = datetime.strptime(
                rec.checkout_date, DEFAULT_SERVER_DATE_FORMAT) +\
                relativedelta(days=-1)
            self._cr.execute("""SELECT id FROM room_availability_line
                                WHERE room_category_id=%s AND close=True
                                AND date BETWEEN %s AND %s
