...
from odoo.tools import float_is_zero, float_compare, DEFAULT_SERVER_DATETIME_FORMAT as dt, DEFAULT_SERVER_DATE_FORMAT
                    ...
                    for line in lines:
                        ...
                        if datas is not None:
                            raise ValidationError(_('''Room Reserved!,
                                                    %s rooms have already reserved for another customer,
                                                    Kindly select another room''') % (line.room_number_id.name))
                        new_room_list.append(line.room_number_id.id)
                        if line.room_number_id.id not in reserved_room_number_list:
                            move_lines.append((0, 0, {'room_number_id': line.room_number_id.id,
                                                      'folio_line_id': line.id}))
                        if line.order_line_id:
                            invoice_ids = [
                                res_line.invoice_lines_ids.id for res_line in line.reservation_lines_ids]
                            invoice_ids = [
                                x for x in invoice_ids if x != False]
                            if invoice_ids:
        ...
        for rec in self:
            if rec.checkin and rec.checkout:
                checkin_date = datetime.strptime(
                    str(rec.checkin), DEFAULT_SERVER_DATE_FORMAT)
                checkout_date = datetime.strptime(
                    str(rec.checkout), DEFAULT_SERVER_DATE_FORMAT)
                dur = checkout_date - checkin_date
                rec.stay_days = dur.days

    @api.depends('state', 'reservation_line.invoice_lines_ids', 'reservation_line.invoice_status')
    def _compute_invoice(self):
        invoice_obj = self.env['account.invoice']
        for rec in self:
            invoice_ids = rec.reservation_line.mapped('invoice_lines_ids').mapped(
                'invoice_id').filtered(lambda r: r.type in ['out_invoice', 'out_refund'])
            refunds = invoice_ids.search([('origin', 'like', rec.name),
                                          ('company_id', '=', rec.company_id.id)]
                                         ).filtered(lambda r: r.type in ['out_invoice', 'out_refund'])
            invoice_ids |= refunds.filtered(lambda r: rec.name in [origin.strip()
...
#            line_invoice_status = []
            if invoice_ids:
                for inv in invoice_ids:
                    refund_ids += invoice_obj.search([('type', '=', 'out_refund'),
                                                      ('origin', '=', inv.number),
                                                      ('origin', '!=', False),
            ...
            if not line_invoice_status:
                invoice_status = 'no'
            elif any(invoice_status == 'to_invoice' for invoice_status in line_invoice_status):
    ...
    def default_get(self, fields):
        ...
        if res.get('checkin') and res.get('checkout') and\
            ...
            checkin = datetime.strptime(
                res.get('checkin'), DEFAULT_SERVER_DATE_FORMAT)
    ...
    def compute_check_in_out(self):
        for rec in self:
            ...
            rec.update({
                'checkin': checkin_list and min(checkin_list)
                or datetime.today(),
    ...
    company_id = fields.Many2one('res.company', 'Hotel', readonly=True, states={'draft': [('readonly', False)]},
    ...
    checkin = fields.Date('Checkin-Date', readonly=True,
                          compute="compute_check_in_out",
                          store=True, track_visibility='always')
    checkout = fields.Date('Checkout-Date', readonly=True,
                           compute="compute_check_in_out",
                           store=True, track_visibility='always')
    adults = fields.Integer('Adults', size=64, readonly=True, default=1,
                            states={'draft': [('readonly', False)]},
                            help='List of adults there in guest list. ')
    children = fields.Integer('Children', size=64, readonly=True,
                              states={'draft': [('readonly', False)]},
                              help='Number of children there in guest list.')
    reservation_line = fields.One2many('hotel_reservation.line', 'reservation_id',
                                       'Reservation Line',
    ...
    folios_ids = fields.Many2many('hotel.folio', string='Folio',
                                  compute="_get_folio")
    folio_count = fields.Integer('Folio Count', compute="_get_folio")
    dummy = fields.Datetime('Dummy')
    service_lines = fields.One2many('hotel.reservation.service.line',
                                    'reservation_id',
                                    readonly=True, copy=False,
                                    states={'draft': [('readonly', False)]},
    ...
    amount_total = fields.Monetary(string='Total', store=True,
                                   readonly=True, compute='_amount_all',
                                   track_visibility='always')
    room_move_ids = fields.One2many('hotel.room.move', 'reservation_id',
    ...
    def check_adults_details(self):
        for rec in self:
            if rec.company_id.adults_details_validation and rec.adults != len(rec.reservation_adults_ids):
            ...
            for room in rec.reservation_line.mapped('room_id'):
                ...
                for line in lines:
                    if line.room_number_id:
                        rec._cr.execute("""SELECT room_number_id FROM
                                hotel_room_move_line WHERE (%s,%s) OVERLAPS
                                (check_in, check_out) AND state != 'cancel' AND
                ...
                dates_list.append(line.checkin)
                dates_list.append(line.checkout)
        ...
        for reservation in self:
            ...
            for room in reservation.reservation_line.mapped('room_id'):
                lines = reservation_line_obj.search([('room_id', '=', room.id),
                                                     ('reservation_id', '=', reservation.id)])
                room_no_ids = [
                    line.room_number_id.id for line in lines if line.room_number_id]
...
#            total_room = sum(line.room_qty for line in reservation.room_reservation_line_ids)
            wiz = self.env['hotel.folio.room.allocation'].with_context({'from_reservation': True}).create({'reservation_id': reservation.id,
    ...
    def open_folio_view(self):
        ...
        if len(folios) > 1:
            ...
            action['views'] = [(self.env.ref('hotel.view_hotel_folio1_form').id,
                                'form')]
    ...
    def action_invoice_create(self):
        ...
        for rec in self:
            ...
            for line in rec.reservation_line.sorted(key=lambda l: l.qty_to_invoice < 0):
                i = 1
                while i <= line.qty:
...
class HotelReservationLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            taxes = line.tax_id.compute_all(price, line.reservation_id.currency_id, line_qty,
                                            product=line.room_id.product_id, partner=line.reservation_id.partner_shipping_id)
            line.update({
    ...
    @api.depends('invoice_lines_ids.invoice_id.state', 'invoice_lines_ids.quantity')
    ...
    def _compute_invoice_status(self):
        ...
        for line in self:
            ...
            elif float_compare(line.qty_invoiced, (line.qty * line.stay_days), precision_digits=precision) == 0:
    ...
    room_id = fields.Many2one('hotel.room', 'Room', copy=False,
                              ondelete="restrict", required=True)
    room_number_id = fields.Many2one('hotel.room.number', 'Room Numbers',
                                     copy=False, domain="[('room_id', '=', room_id),\
                                                          ('state', '=', 'available')]")
    price_unit = fields.Float('Room Rate', required=True,
                              digits=dp.get_precision('Product Price'),
                              default=0.0)
    discount = fields.Float('Discount (%)', digits=dp.get_precision('Discount'),
                            default=0.0)
    room_status = fields.Selection(related='room_id.status', string='Status',
                                   readonly=True)
    qty = fields.Float(string='Quantity',
                       digits=dp.get_precision('Product Unit of Measure'),
                       readonly=False, required=True, default=1.0)
    stay_days = fields.Integer("Nights", copy=False, readonly=True,
    ...
    company_id = fields.Many2one(related='reservation_id.company_id',
                                 string='Company', store=True, readonly=True)
    checkin = fields.Date("Checkin")
    checkout = fields.Date("Checkout")
    folio_lines_ids = fields.Many2many('hotel.folio.line',
                                       'hotel_folio_line_reservation_line_rel',
                                       'reservation_line_id', 'folio_line_id',
                                       string='Reservation Lines',
                                       readonly=True, copy=False)
    invoice_lines_ids = fields.Many2many('account.invoice.line',
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            line.tax_id = fpos.map_tax(
                taxes, line.room_id, line.reservation_id.partner_shipping_id) if fpos else taxes
            ...
            if pricelist_item.pricelist_id.discount_policy == 'without_discount':
                while pricelist_item.base == 'pricelist' and pricelist_item.base_pricelist_id and pricelist_item.base_pricelist_id.discount_policy == 'without_discount':
                    price, rule_id = pricelist_item.base_pricelist_id.with_context(
                        uom=uom.id).get_product_price_rule(product, qty, self.reservation_id_id.partner_id)
                    pricelist_item = PricelistItem.browse(rule_id)
    ...
    def _get_display_price(self, room):
        if self.reservation_id.pricelist_id.discount_policy == 'with_discount':
            return room.with_context(pricelist=self.reservation_id.pricelist_id.id).price
        room_context = dict(self.env.context, partner_id=self.reservation_id.partner_id.id,
                            date=self.reservation_id.date_order, uom=self.room_id.product_id.uom_id.id)
        final_price, rule_id = self.reservation_id.pricelist_id.with_context(room_context).get_product_price_rule(
            self.room_id.product_id, self.qty or 1.0, self.reservation_id.partner_id)
        base_price, currency_id = self.with_context(room_context)._get_real_price_currency(
            room, rule_id, self.qty, self.room_id.product_id.uom_id, self.reservation_id.pricelist_id.id)
        if currency_id != self.reservation_id.pricelist_id.currency_id.id:
            base_price = self.env['res.currency'].browse(currency_id).with_context(
                room_context).compute(base_price, self.reservation_id.pricelist_id.currency_id)
    ...
    def onchange_room_id(self):
        ...
        if self.reservation_id.pricelist_id and self.reservation_id.partner_id:
            vals['price_unit'] = self.env['account.tax'
                                          ]._fix_tax_included_price_company(self._get_display_price(room),
    ...
    def room_qty_change(self):
        ...
        if self.reservation_id.pricelist_id and self.reservation_id.partner_id:
            ...
            self.price_unit = self.env['account.tax']._fix_tax_included_price_company(self._get_display_price(room),
                                                                                      room.taxes_id, self.tax_id,
                                                                                      self.company_id)
    ...
    def _prepare_invoice_line(self, qty):
        self.ensure_one()
        res = {}
        account = self.room_id.product_id.property_account_income_id or self.room_id.product_id.categ_id.property_account_income_categ_id
        if not account:
    ...
    def check_reservation_rooms(self):
        ...
        for line in self:
            available_room_qty = line.get_available_room_qty(
                line.room_id, line.checkin, line.checkout)
            if line.qty > available_room_qty:
                raise ValidationError(_('You tried to Reservation with "%s" room'
                                        ' but this type room only %s left.'
                                        ) % (line.room_id.name, available_room_qty))
...
class HotelServiceLine(models.Model):
    ...
    def _compute_amount(self):
        for line in self:
            price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            taxes = line.tax_id.compute_all(price, line.reservation_id.currency_id,
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            taxes = line.product_id.taxes_id.filtered(
                lambda r: not line.company_id or r.company_id == line.company_id)
            line.tax_id = fpos.map_tax(
                taxes, line.product_id, line.reservation_id.partner_shipping_id) if fpos else taxes
    ...
    def _get_real_price_currency(self, product, rule_id, qty, uom, pricelist_id):
        ...
        if rule_id:
            ...
            if pricelist_item.pricelist_id.discount_policy == 'without_discount':
                while pricelist_item.base == 'pricelist' and pricelist_item.base_pricelist_id and pricelist_item.base_pricelist_id.discount_policy == 'without_discount':
                    price, rule_id = pricelist_item.base_pricelist_id.with_context(
                        uom=uom.id).get_product_price_rule(product, qty, self.reservation_id.partner_id)
                    pricelist_item = PricelistItem.browse(rule_id)
    ...
    def _get_display_price(self, product):
        if self.reservation_id.pricelist_id.discount_policy == 'with_discount':
            return product.with_context(pricelist=self.reservation_id.pricelist_id.id).price
        ...
        final_price, rule_id = self.reservation_id.pricelist_id.with_context(
            product_context).get_product_price_rule(self.product_id, self.qty or 1.0, self.reservation_id.partner_id)
        base_price, currency_id = self.with_context(product_context)._get_real_price_currency(
            product, rule_id, self.qty, self.product_uom, self.reservation_id.pricelist_id.id)
        if currency_id != self.reservation_id.pricelist_id.currency_id.id:
            base_price = self.env['res.currency'].browse(currency_id).with_context(
                product_context).compute(base_price, self.reservation_id.pricelist_id.currency_id)
        return max(base_price, final_price)
